...
const ItemHome = ({ info }) => {
  ...
  return (
    <NavLink to={path} className='relative flex flex-col group hover:border-orange-400 bg-slate-600 md:h-[20vw] md:w-[20vw] h-full w-full md:m-5 cursor-pointer border border-blue-800'>
      ...
      <div className='absolute md:bottom-[10%] bottom-[1%] left-1/2 transform -translate-x-1/2 text-white bg-slate-700 border border-orange-400 md:p-4 p-1 text-center max-h-[40%] opacity-100 whitespace-normal md:whitespace-nowrap'>
        {info.info.name}
      </div>
    </NavLink>
  );
